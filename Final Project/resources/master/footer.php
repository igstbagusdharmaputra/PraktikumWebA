<?php if (FIRST_PART == "home") { ?>
            ...
            <footer class="main-footer">
                <div class="container">
                    <strong>Copyright &copy; <label id="year-copy"></label> <a href="<?php BASE_URL ?>" target="_blank">Sistem Informasi Perpustakaan</a>.</strong> All rights reserved.
                </div>
                <!-- /.container -->
            </footer>
        <?php } else if (FIRST_PART == "dashboard") { include_once '../app/modal.php'; ?>
            <script>
                $(function() {
                    ...
                    var res = str === undefined ? '' : str.substr(0, 15);
                    $('span.hidden-xs').html(res);
                    var name_account = $('#account').find('p').html();
                    var nm = name_account === undefined ? '' : name_account.substr(0,15);
    ...
    <?= FIRST_PART == "register" ? '<script src="'.BASE_URL.'assets/js/page/register.js"></script>' : '' ?>
    <?php if (FIRST_PART == "home") { ?> 
        <script>
            $(function() {
